...
```json
{
  ...
  "context": {
    "lastGeneration": {
      ...
      "prompt": "переформулированный промпт прошлой генерации (опц., default '')",
      "hasImage": "boolean — есть ли готовый результат-изображение (default false)"
    }
  }
}
```
`context.lastGeneration` опционален — клиент передаёт его для follow-up правок («сделай теперь зиму»).
...
```json
{
  "data": {
    ...
    "prompt": "переформулированный промпт под модель (для edit — описание изменения)",
    "parameters": { "prompt": "...", "...": "извлечённые параметры под выбранную модель" }
  }
}
```
```json
{ "data": { "action": "clarify", "question": "уточняющий вопрос пользователю" } }
```
```json
{ "data": { "action": "notice", "message": "текст для показа в чате (напр. про STT)" } }
```
Логика: Kimi (`kimi-k2-6`) классифицирует намерение и переформулирует промпт (шаг 1), затем для `generate`/`edit` извлекает **структурированные параметры** под выбранную модель (шаг 2, валидируются той же Zod-схемой, что и `/api/studio/generate`). Модель выбирается **хаотично** из нужной категории (изображения — принудительно `nano-banana-2`) и только в режиме без обязательных загрузок (t2v/t2i/generate/tts) — временно; позже добавим полноценную логику оркестрации.

**Контекст диалога** (`context.lastGeneration`): при наличии прошлой генерации Kimi может вернуть:
- `edit` — пользователь явно дорабатывает прошлый результат («сделай зиму», «добавь снеговика», «сделай длиннее»). Для **фото** клиент подставляет прошлое изображение как референс (`imageInput` → KIE `image_input`); для **видео** (и неготового фото) — несёт контекст текстом «прошлая сцена + изменение». В обоих случаях привязывается `parentGenerationId` (remix-цепочка). Работает и в авто-чате, и в ручных режимах «Фото»/«Видео».
- `clarify` — неоднозначно (править прошлое ИЛИ создать новое): задаётся уточняющий вопрос, генерация не запускается; ответ пользователя снова уходит в оркестратор.

STT (Whisper) распознаётся, но возвращается `notice`: пайплайна транскрипции из чата пока нет (нужен аудиофайл). Клиент автоматически переключает вкладку и запускает генерацию.
...
| POST | `/api/studio/lyrics` | 60/min | Сгенерировать/отредактировать текст песни (LLM) |
...
### POST /api/studio/lyrics

Интерактивная генерация текста песни (через LLM `kimi-k2-6`) для музыкального флоу:
сначала показываем текст, даём отредактировать, и только после принятия генерируем музыку.

```json
{
  "prompt": "string (required, описание трека)",
  "current": "string (optional, текущий текст — для правки)",
  "instruction": "string (optional, что изменить — для правки)"
}
```
Если переданы `current` + `instruction` → режим ПРАВКИ (меняется только указанная часть,
остальное сохраняется). Иначе → генерация с нуля по `prompt`.

Ответ:
```json
{ "data": { "text": "полный текст песни с тегами секций", "title": "название или null" } }
```
...
```json
{
  "data": {
    ...
    "errorCode": null,
    "errorMessage": null,
    "lyrics": "текст песни (Suno) или null",
    "lyricsTitle": "название трека или null"
  }
}
```
